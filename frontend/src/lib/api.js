...
export const cyclePermissionMode = (agentId, steps = 1, target = null) => {
  const q = target ? `steps=${steps}&target=${target}` : `steps=${steps}`;
  return request(`/api/agents/${agentId}/cycle-permission-mode?${q}`, { method: "POST" });
};
