...
- **Cross-session reference** — tell an agent "check AgentHive session `<session_id>`" and it can read another agent's full conversation via a built-in [MCP server](orchestrator/mcp_server.py). Great for handing off context or reviewing a previous attempt.
